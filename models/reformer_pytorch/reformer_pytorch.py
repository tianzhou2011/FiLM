...
class LSHAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, query_len = None, input_mask = None, input_attn_mask = None, pos_emb = None, **kwargs):
        B, L_Q, H, D = queries.shape
        _, L_K, _, _ = keys.shape
        qk = queries.reshape([B, L_Q, -1])
        v = values.reshape([B, L_Q, -1])
...
class Reformer(nn.Module):
    def __init__(self, dim, depth, heads = 8, dim_head = None, bucket_size = 32, n_hashes = 8, ff_chunks = 100, attn_chunks = None, causal = False, weight_tie = False, lsh_dropout = 0., ff_dropout = 0., ff_activation = None, ff_mult = 4, ff_glu = False, post_attn_dropout = 0., layer_dropout = 0., lsh_attend_across_buckets = True, lsh_allow_duplicate_attention = True, random_rotations_per_head = False, use_scale_norm = False, use_rezero = False, use_full_attn = False, full_attn_thres = 0, reverse_thres = 0, num_mem_kv = 0, one_value_head = False, n_local_attn_heads = 0, pkm_layers = tuple(), pkm_num_keys = 128):
        super().__init__()
        self.dim = dim
        self.depth = depth
